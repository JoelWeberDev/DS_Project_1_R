...
|------------|------------|-----------|---------------------|---------------|
|169025972   |Joel        |Weber      |webe5972@mylaurier.ca| ln 1 - ln 308 |
|            |            |           |                     |               |
|            |            |           |                     |               |
|            |            |           |                     |               |
|            |            |           |                     |               |
...
```{r error=TRUE, tags=c()}
# Get the count of the number of distinct storms between 1975 and 2021 when the last data was released. Then turn that into a data frame.
(storm_rough <- storms |>
   select(name, year) |>
   distinct() |>
   summarise(start_year = min(year), end_year = max(year), count = n())
)
```
```{r error=TRUE}
...
```{r error=TRUE, tags=c()}
(storm_range <- storms |> 
   summarize(.by = year, min_wind = min(wind), max_wind = max(wind), min_lat = min(lat), max_lat = max(lat)) |> 
...
(cat_status_plot <- storms |>
  ggplot(mapping = aes(x = category, y = status)) + geom_count()
)
...
(status_wind_plot <- storms |>
  ggplot(mapping = aes(x = wind, y = status)) + geom_jitter(alpha = 0.3) +
  status_wind_labels
)
```
```{r error=TRUE}
...
(category_jitter_plot <- new2_storms |>
  ...
  |> ggplot(
      mapping = aes(x = wind, y = factor_cat, color = factor_cat)
    ) +                                   
    geom_jitter() +
...
(category_ridges_plot <- new2_storms |>
  filter(status == "hurricane")
  |> ggplot(
      mapping = aes(x = wind, y = factor_cat, color = factor_cat, fill = factor_cat)
    ) +                                   
    geom_density_ridges(
      alpha = 0.5,
      show.legend = FALSE
...
(pressure_box_plot <- new2_storms |>
  ggplot(
    mapping = aes(x = pressure, y = factor_cat)
  ) +                                             
    geom_boxplot() +
  pressure_plot_labels +
...
(storm_count_plot <- storms_by_storm |>
  ggplot(
    mapping = aes(x = year, fill = factor_max_cat)
    ) +
...
(storms_in_year_plot <- storms_by_storm |>
    filter(year == 2021)
  |> ggplot(
      mapping = aes(x = first_day, y = name, color = factor_max_cat)
    ) +                                 
  geom_point() +
  geom_segment(
    mapping = aes(xend = last_day, yend = name, color = factor_max_cat)
    ) +                                 
  scale_x_date(
    name = NULL,
    date_labels = "%b"
...
(duration_vs_category_bar_plot <- storms_by_storm |>
  ggplot(
    mapping = aes(x = days, fill = factor_max_cat)
    ) +              
    geom_bar(color = "black") +
  theme(legend.position = "bottom") +            
  guides(
